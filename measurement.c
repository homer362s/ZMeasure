...
static void updateSweepDisplay(int panel, unsigned int index)
{
...
static void editMeasurementStep(MeasStep* measStep, unsigned int index)
{
...
int CVICALLBACK measvar_CB (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	if (event == EVENT_COMMIT)
	{
		...
		int varIndex = getMeasVarIndex(measStep, measVar);
		if (!measVar) {
			return 0;
		}
...
int CVICALLBACK measstep_CB (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	...
	if (event == EVENT_COMMIT)
	{
		...
		if (control == STEPP_STEP && measVar) {
			...
			measStep->nPoints = (unsigned int) ((stop-start)/(double)step);    	  
		}
